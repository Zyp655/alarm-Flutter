...
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "frames_v2")
...
def extract_frames_from_clip(video_path, output_path, n_frames=FRAMES_PER_CLIP):
    ...
    for i, idx in enumerate(indices):
        ...
        fname = f"{Path(video_path).stem}_f{i:02d}.jpg"
        fpath = os.path.join(output_path, fname)
        cv2.imwrite(fpath, face, [cv2.IMWRITE_JPEG_QUALITY, 95])
        saved.append(fpath)
...
def process_split(split_name):
    ...
    processed = 0
    skipped = 0
    confusion_dist = {0: 0, 1: 0, 2: 0, 3: 0}
    ...
    for si, subject in enumerate(subjects):
        ...
        for clip_folder in clips:
            ...
            confusion_dist[labels['confusion']] += 1

            is_confused = 1 if labels['confusion'] >= CONFUSION_THRESHOLD else 0
    ...
    print(f"  Confusion distribution: {confusion_dist}")
    return records


def main():
    print("=" * 60)
    print("  DAiSEE Frame Extraction v2")
    ...
    for split in ['Train', 'Test', 'Validation']:
        ...
        confused = sum(1 for r in records if r['is_confused'] == 1)
        not_confused = len(records) - confused
        print(f"  -> {len(records)} frames (confused={confused}, not_confused={not_confused})")
    ...
    if all_records:
        ...
        print(f"\nTotal frames: {len(df)}")
        print(f"Confused (confusion>=2): {df['is_confused'].sum()}")
        print(f"Not confused (confusion<2): {len(df) - df['is_confused'].sum()}")
